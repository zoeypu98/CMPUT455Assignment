...
import numpy as np
from transposition_table import TranspositionTable, hash_function
import re
...
class GtpConnection:
    ...
    def negamaxBoolean(self, state, tt):
        ...
        legal_moves = GoBoardUtil.generate_legal_moves(state, color)
        ...
        for m in legal_moves:
            ...
            state.play_tempmove(state, m, color)

            success = self.negamaxBoolean(state, tt)

            state.undo_tempmove(state, m, color)

            if len(success) == 1:  # opponent is loosing, win for current player
                result = [True, m]
                return result
    ...
    def call_search(self, state):
        tt = TranspositionTable()  # use separate table for each state
    ...
    def solve_cmd(self, args):
        ...
        # self.board.board  mutable numpy array board
        state = self.board
        start = time.process_time()
        result = self.call_search(state)  # using board object
        time_used = time.process_time() - start

        if time_used > self.seconds:
            self.respond("unknown")
            return False
        # while self.seconds:
        #     result = self.call_search(state)
        #     time.sleep(1)
        #     seconds -= 1
        ...
        if result[0]:
            point = result[1]
            coord = point_to_coord(point, state.size)
            best_move = format_point(coord)

            best_move_col = best_move[0].lower()
            # self.respond("best move{}".format(best_move))
            best_move_row = best_move[1]

            self.respond("{} {}{}".format(int_to_color[color], best_move_col, best_move_row))
            return [int_to_color[color], best_move]

        else:  # respond with the opponent the win
            ...
            self.respond("{}".format(int_to_color[color]))
            return [int_to_color[color]]
    def solve_no_print(self, args):
        int_to_color = {BLACK: 'b', WHITE: 'w'}
        color = self.board.current_player
