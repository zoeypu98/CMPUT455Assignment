...
class GtpConnection:
    ...
    def gogui_rules_legal_moves_cmd(self, args):
        ...
        for emty_p in empty_points:

            legal, msg = self.board.is_legal(emty_p, color)

            if legal:

                move_coord = point_to_coord(emty_p, self.board.size)
        ...
        if moves_str != "":
            self.respond("{}".format(moves_str))
        else:
            self.respond("")

        return moves

    def legal_move_no_print(self):
        empty_points = self.board.get_empty_points()
        color = self.board.current_player
    ...
    def play_cmd(self, args):
        ...
        try:
            ...
            if args[1].lower() == "pass":  # pass is illegal
            ...
            value, er_msg = self.board.is_legal(move, color)
            if not value:
                self.respond('illegal move: "{} {}"'.format(board_color, board_move) + er_msg)
                return
            else:
    ...
    def genmove_cmd(self, args):
        ...
        #  No legal moves
        if self.legal_move_no_print() == []:
            # the response of genmove should be: resign
            self.respond("resign")
            return
        else:
            # Before the end of a game, genmove will play a random legal move
            board_color = args[0].lower()  # b a1
            color = color_to_int(board_color)
            move = self.go_engine.get_move(self.board, color)
            move_coord = point_to_coord(move, self.board.size)
            move_as_string = format_point(move_coord)
            if self.board.is_legal(move, color):
                print(self.board.is_legal(move, color))
                self.gogui_rules_legal_moves_cmd(args)
...
def format_point(move):
    ...
    if not 0 <= row < MAXSIZE or not 0 <= col < MAXSIZE:
        raise ValueError
    return column_letters[col - 1] + str(row)  # B2
